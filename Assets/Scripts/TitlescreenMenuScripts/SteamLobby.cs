...
public class SteamLobby : MonoBehaviour
{
    ...
    private void OnLobbyCreated(LobbyCreated_t callback)
    {
        ...
        SteamMatchmaking.SetLobbyData(
            ...
            "GameName",
            "StalksStalksStalks");
        // Provide number of years as lobby metadata
        SteamMatchmaking.SetLobbyData(
            new CSteamID(callback.m_ulSteamIDLobby),
            "NumberOfYears",
            numberOfYears.ToString());
        SteamMatchmaking.SetLobbyData(
            new CSteamID(callback.m_ulSteamIDLobby),
            "LoansEnabled",
            loansEnabled.ToString());
    ...
    public void CreateNewLobby(ELobbyType lobbyType, int maxPlayers, int years, bool areLoansEnabled)
    {
        ...
        if (maxPlayers > 0)
        {
            ...
            SteamMatchmaking.CreateLobby(lobbyType, maxNumberOfPlayers);
        }            
    }
}
